...
from duels_training.bridge import action_log_probs, sample_action, compute_entropies
...
def test_action_log_prob_big_batch():
    ...
    actions = Action(
        ...
        inventory_slot=torch.randint(low=0, high=5, size=[batch_size])
    )

    model_output = torch.randn([batch_size, 21]) * 5
...
def test_action_log_prob_individual():
    model_output = torch.cat([
        ...
        torch.log(torch.tensor([0.2, 0.55, 0.1, 0.05, 0.07, 0.03]))
...
def test_compute_entropies():
    ...
    expected_entropy = (0.8018 + 0.9433 + 0.6109 + 0.1985 + 2.5176 + 2.1121 + 0.9433 + 1.3221) / 8
...
def test_sample_action():
    ...
    assert len(list(filter(lambda x: x == 0, inventory_slots))) == pytest.approx(200, abs=50)
    ...
    assert len(list(filter(lambda x: x == 3, inventory_slots))) == pytest.approx(50, abs=20)
    assert len(list(filter(lambda x: x == 4, inventory_slots))) == pytest.approx(70, abs=20)
    assert len(list(filter(lambda x: x == 5, inventory_slots))) == pytest.approx(30, abs=20)

    assert statistics.mean(delta_yaws) == pytest.approx(4, rel=0.1)
    assert statistics.mean(delta_pitches) == pytest.approx(-5, rel=0.1)
