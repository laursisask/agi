...
def compute_log_prob_dists(model_output):
    ...
    assert model_output.shape == (batch_size, 18)
    ...
    hotbar_dist = F.log_softmax(model_output.narrow(dim=1, start=15, length=3), dim=1)
...
def sample_action(model_output):
    ...
    clicking_action = torch.multinomial(torch.exp(clicking_dist), num_samples=1)
    # 0 - attacking
    # 1 - using
    # 2 - nothing

    hotbar_slot = torch.multinomial(torch.exp(hotbar_dist), num_samples=1)
